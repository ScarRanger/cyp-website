  ...
  try {
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        // Check if ticket is locked by another session
        if (ticketData?.status === 'soft-locked' && ticketData?.sessionId !== sessionId) {
          ...
          if (now - lockedTime < LOCK_EXPIRY) {
            throw new Error('Ticket is currently reserved by another user');
          }
          // Lock expired, continue with locking
        }

        // Atomically update the ticket status
      ...
      if (transactionError.message === 'Ticket not found') {
        return NextResponse.json(
          { error: 'Ticket not found' },
          { status: 404 }
        );
      }
      if (transactionError.message === 'Ticket already sold') {
        return NextResponse.json(
          { error: 'Ticket already sold' },
          { status: 400 }
        );
      }
      if (transactionError.message === 'Ticket is currently reserved by another user') {
        return NextResponse.json(
          { error: 'Ticket is currently reserved by another user' },
          { status: 400 }
        );
      }
      throw transactionError; // Re-throw unknown errors
    }
  } catch (error) {
